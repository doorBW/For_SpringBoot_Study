...
- 캐시가 없을 때
    ...
    - 사용자에게 안좋다!
- 캐시 적용
    - 헤더에 `cache-control: max-age=~` -> 캐시가 유효한 시간
    - 응답 결과를 캐시에 저장
    - 두번째요청때 캐시유효시간 검증이 되면 네트워크를 안타고 캐시에서 가져온다.
- 캐시 시간 초과
    - 당연히 다시 요청
    - 응답 결과를 다시 캐시에 저장
    - 캐시 유효 시간이 초과하면, 서버를 통해 데이터를 다시 조회하고, 캐시를 갱신한다.
    - 근데 안바뀌었을때 다시 다 다운받는건 아깝다.
    - -> 어떻게 해결할까?
### 검증 헤더와 조건부요청1
    - 캐시 유효시간이 초과해서 서버에 다시 요청하면 둘 중하나다.
        1. 서버에서 기존 데이터를 변경한 상태
        2. 서버에서 기존 데이터를 변경하지 않은 상태
    - 캐시 만료후에도 서버에서 데이터를 변경하지 않으면(2번상태이면)
    - 클라이언트의 데이터와 서버의 데이터가 같다는 사실이 확인 필요함!
    - -> 검증 헤더 추가
    - 응답 헤더에 `Last-Modified` 필드로 마지막 수정값을 넣고, 이걸 캐시에 같이 넣는다.
    - 캐시 유효 시간이 초과되었을때, 요청 헤더에 `if-modified-since` 필드에, 캐시에 저장한 `Last-Modified` 값을 넣는다.
    - 서버가 요청을 받아서 `if-modified-since`랑 서버가 가지고 있는 데이터 최종 수정일을 보고 판단한다.
    - 데이터가 아직 수정되지 않았으면 `304 Not Modified` 응답을 준다. -> **HTTP Body가 없음!**
    - 클라이언트는 이걸 통해 캐시에 있는 응답결과를 갱신하여 다시 사용
    - 검증헤더: `Last-Modified`
...
### 검증 헤더와 조건부요청2
    - `Last-Modified`,`if-modified-since` 단점
        ...
        - 날짜 기반의 로직 사용
        - 데이터를 수정해서 날짜가 다르지만, 같은 데이터를 수정해서 데이터 겨로가가 똑같은 경우
        - 서버에서 별도의 캐시로직을 관리하고 싶은 경우
    - `ETag`, `If-None-Match`
    - 캐시용 데이터에 임의의 고유한 버전 이름을 달아둠
        - ex. ETag: "v1.0", ETag: "a2jiodwjekjl3"
    - 데이터가 변경되면 이 이름을 다시 바꾸어서 변경함(Hash를 다시 생성)
    - 단순하게 ETag만 보내서 같으면 유지, 다르면 다시 받기!
    - -> **케시 제어 로직은 서버에서 완전히 관리**
### 캐시와 조건부 요청 헤더
