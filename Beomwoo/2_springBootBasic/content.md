...
- 종료메서드에 추론기능이 있다. -> 디폴트로 close, shutdown 이라는 이름의 종료 메서드를 자동으로 호출 (사용하기 싫으면 공백으로 두면 된다.)

### 애노테이션 @PostConstruct, @PreDestory
- 이걸사용하자. 스프링에서도 이걸 권장한다.
- 패키지에서, javax로 시작하면 자바에서 공식적으로 지원하는거다. -> 자바 표준이기에 스프링이 아닌 다른 컨테이너에서도 동작한다.
- 컴포넌트 스캔과 잘 어울린다.
- 유일한 단점은 외부 라이브러리에는 적용하지 못한다는 것. 외부 라이브러리에 대한 초기화, 종료를 해야하면 @Bean의 기능을 사용하자.
